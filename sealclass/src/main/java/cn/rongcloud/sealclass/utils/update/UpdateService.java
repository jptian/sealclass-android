...
public class UpdateService extends Service {
    ...
    private static String channelId = "cn.rongcloud.sealclass.update";
...
//    }
    ...
    private Intent installIntent(String path) {
        ...
        try{
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                apkUri = FileProvider.getUriForFile(Utils.getContext(),
                        BuildConfig.APPLICATION_ID + ".provider", apkFile);
            } else {
            ...
            List<ResolveInfo> resolveLists = Utils.getContext().getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            // 然后全部授权
            for (ResolveInfo resolveInfo : resolveLists){
                String packageName = resolveInfo.activityInfo.packageName;
                Utils.getContext().grantUriPermission(packageName, apkUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
